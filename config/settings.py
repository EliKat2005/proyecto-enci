...
DATABASES = {
    "default": {
        ...
        "TEST": {
            # Usa una BD de pruebas existente (no la crea automáticamente)
            "CREATE_DB": False,
            "NAME": os.getenv("DB_TEST_NAME", "enci_test"),
        },
    }
}
...
# --- CONFIGURACIÓN DE PYTEST (MariaDB) ---
if "pytest" in os.sys.modules or "PYTEST_CURRENT_TEST" in os.environ:
    # Usa credenciales de pruebas si están definidas
    DATABASES["default"]["NAME"] = os.getenv(
        "DB_TEST_NAME", DATABASES["default"]["TEST"]["NAME"]
    )  # enci_test por defecto
    DATABASES["default"]["USER"] = os.getenv("DB_TEST_USER", DATABASES["default"]["USER"])
    DATABASES["default"]["PASSWORD"] = os.getenv(
        "DB_TEST_PASSWORD", DATABASES["default"]["PASSWORD"]
    )

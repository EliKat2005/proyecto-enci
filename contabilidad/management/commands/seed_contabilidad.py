...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def crear_cuenta(
            codigo, descripcion, tipo, naturaleza, padre=None, aux=False, estado_situacion=True
        ):
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f" ✓ Cuenta creada {codigo} ({'Aux' if aux else 'Grupo'})")
                )
            return obj
        ...
        activo_corriente = crear_cuenta(
            "1.1", "ACTIVO CORRIENTE", "Activo", "Deudora", padre=activo, aux=False
        )
        caja = crear_cuenta("1.1.01", "Caja", "Activo", "Deudora", padre=activo_corriente, aux=True)
        banco = crear_cuenta(
            "1.1.02", "Banco", "Activo", "Deudora", padre=activo_corriente, aux=True
        )

        ingreso = crear_cuenta(
            "4", "INGRESOS", "Ingreso", "Acreedora", aux=False, estado_situacion=False
        )
        ventas = crear_cuenta(
            "4.1",
            "VENTAS",
        ...
        resultados_ejercicio = crear_cuenta(
            "3.3", "Resultados del Ejercicio", "Patrimonio", "Acreedora", padre=patrimonio, aux=True
        )
        ...
        try:
            _, _ = AsientoService.crear_asiento(
                ...
                lineas=[
                    {
                        "cuenta_id": caja.id,
                        "detalle": "Entrada caja",
                        "debe": Decimal("100.00"),
                        "haber": Decimal("0.00"),
                    },
                    {
                        "cuenta_id": ventas_locales.id,
                        "detalle": "Venta",
                        "debe": Decimal("0.00"),
                        "haber": Decimal("90.00"),
                    },
            ...
            self.stdout.write(
                self.style.ERROR("ERROR: Se creó asiento desbalanceado (esto no debería ocurrir).")
            )
        ...
        try:
            _, _ = AsientoService.crear_asiento(
                ...
                lineas=[
                    {
                        "cuenta_id": activo_corriente.id,
                        "detalle": "Movimiento inválido",
                        "debe": Decimal("50.00"),
                        "haber": Decimal("0.00"),
                    },
                    {
            ...
            self.stdout.write(
                self.style.ERROR("ERROR: Se permitió movimiento en cuenta no auxiliar.")
            )
        except Exception as e:
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Asiento bancarizado correcto creado # {asiento_ok.numero_asiento}"
                )
            )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"Fallo creación de asiento bancarizado correcto: {e}")
            )
        ...
        try:
            asiento_gasto = _, _ = AsientoService.crear_asiento(
                ...
                lineas=[
                    {
                        "cuenta_id": sueldos.id,
                        "detalle": "Pago sueldos",
                        "debe": Decimal("800.00"),
                        "haber": Decimal("0.00"),
                    },
                    {
                        "cuenta_id": caja.id,
                        "detalle": "Salida efectivo",
                        "debe": Decimal("0.00"),
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Asiento gasto menor creado # {asiento_gasto.numero_asiento}")
            )
        except Exception as e:
        ...
        try:
            er = EstadosFinancierosService.estado_de_resultados(
                empresa=empresa, fecha_inicio=date.today(), fecha_fin=date.today()
            )
        ...
        self.stdout.write(
            "\n"
            + self.style.SUCCESS(
                "Seeding y validaciones contables completadas. Revisa las vistas plan y diario."
            )
        )

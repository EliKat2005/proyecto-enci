...
from contabilidad.models import (
    Empresa, EmpresaPlanCuenta, EmpresaAsiento, 
    EmpresaTransaccion, PeriodoContable
)
from contabilidad.services import LibroMayorService, EstadosFinancierosService
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if errores == 0 and advertencias == 0:
            self.stdout.write(self.style.SUCCESS(
                '✓ VALIDACIÓN EXITOSA - Todos los controles aprobados'
            ))
        elif errores == 0:
            self.stdout.write(self.style.WARNING(
                f'⚠ VALIDACIÓN COMPLETADA - {advertencias} advertencia(s)'
            ))
        else:
            self.stdout.write(self.style.ERROR(
    ...
    def _validar_plan_cuentas(self, empresa):
        ...
        for codigo in codigos_requeridos:
            if not cuentas.filter(codigo=codigo).exists():
                self.stdout.write(self.style.ERROR(
                    f'  ✗ Falta cuenta principal: {codigo}'
                ))
                errores += 1
        
        # Validar naturalezas
        for cuenta in cuentas:
            if cuenta.codigo.startswith('1') or cuenta.codigo.startswith('5') or cuenta.codigo.startswith('6'):
                if cuenta.naturaleza != 'Deudora':
                    self.stdout.write(self.style.ERROR(
                        f'  ✗ Cuenta {cuenta.codigo} ({cuenta.naturaleza}) debe tener naturaleza DEUDORA'
                    ))
                    errores += 1
            elif cuenta.codigo.startswith('2') or cuenta.codigo.startswith('3') or cuenta.codigo.startswith('4'):
    ...
    def _validar_asientos(self, empresa):
        ...
        for asiento in asientos:
            ...
            total_debe = transacciones.aggregate(
                total=Sum('debe')
            )['total'] or Decimal('0')
            
            total_haber = transacciones.aggregate(
                total=Sum('haber')
            )['total'] or Decimal('0')
            ...
            transacciones_invalidas = transacciones.filter(
                debe__gt=0, haber__gt=0
            ).count()
        ...
        if errores == 0:
            self.stdout.write(self.style.SUCCESS(
                f'  ✓ Todos los {total_asientos} asientos cumplen partida doble'
            ))
    ...
    def _validar_balance(self, empresa):
        ...
        if diferencia > Decimal('0.01'):
            self.stdout.write(self.style.ERROR(
                f'  ✗ Balance de comprobación descuadrado: ${diferencia}'
            ))
            errores += 1
        else:
            self.stdout.write(self.style.SUCCESS(
        ...
        cuentas_auxiliares = EmpresaPlanCuenta.objects.filter(
            empresa=empresa,
            es_auxiliar=True
        )
    ...
    def _validar_estados_financieros(self, empresa):
        ...
        if abs(diferencia) > Decimal('0.01'):
            ...
            self.stdout.write(self.style.SUCCESS(
                '  ✓ Balance General cuadrado (Activo = Pasivo + Patrimonio)'
            ))
    ...
    def _validar_periodos(self, empresa):
        ...
        if total_periodos != 12:
            self.stdout.write(self.style.WARNING(
                f'  ⚠ Se esperaban 12 periodos (año completo), '
                f'se encontraron {total_periodos}'
            ))
            advertencias += 1

...
from .models import Grupo, Invitation, Notification, Referral, UserProfile
        ...
        if role == "estudiante":
            ...
            if not codigo:
                raise forms.ValidationError(
                    "Si te registras como estudiante debes ingresar un código de invitación proporcionado por tu docente."
                )
            ...
            if not invitation.is_valid():
                raise forms.ValidationError(
                    "El código de invitación ha expirado o ha alcanzado su límite de uso."
                )
        ...
        if commit:
            ...
            if role == "estudiante":
                UserProfile.objects.create(
                    user=user, rol=UserProfile.Roles.ESTUDIANTE, esta_activo=False
                )
                        ...
                        if invitation.is_valid():
                            ...
                            try:
                                AuditLog.objects.create(
                                    actor=invitation.creator,
                                    target_user=user,
                                    action="student_registered",
                                    description=f"Estudiante {user.username} registrado con código {invitation.code}",
                                )
                            except Exception:
                                pass
                            ...
                            try:
                                ...
                                if admin_emails:
                                    ...
                                    try:
                                        admin_text = render_to_string(
                                            "emails/new_student_admins.txt",
                                            {
                                                "student": user,
                                                "invitation": invitation,
                                                "dashboard_url": "/admin/",
                                    ...
                                    try:
                                        admin_html = render_to_string(
                                            "emails/new_student_admins.html",
                                            {
                                                "student": user,
                                                "invitation": invitation,
                                                "dashboard_url": "/admin/",
                                    ...
                                    try:
                                        ...
                                        try:
                                            AuditLog.objects.create(
                                                actor=None,
                                                target_user=None,
                                                action="email_sent_admins",
                                                description=f"Notificación enviada a admins sobre estudiante {user.username}",
                                            )
                                        except Exception:
                                            pass
                            ...
                            try:
                                ...
                                if docente_email:
                                    ...
                                    from_email = getattr(
                                        settings, "DEFAULT_FROM_EMAIL", None
                                    ) or getattr(settings, "SERVER_EMAIL", "no-reply@example.com")
                                    ...
                                    try:
                                        msg = EmailMultiAlternatives(
                                            subject, text_body, from_email, [docente_email]
                                        )
                                        if html_body:
                                        ...
                                        try:
                                            AuditLog.objects.create(
                                                actor=None,
                                                target_user=invitation.creator,
                                                action="email_sent",
                                                description=f"Notificación enviada a {docente_email} sobre estudiante {user.username}",
                                            )
                                        except Exception:
                                            pass
                                    except Exception as e:
                                        try:
                                            AuditLog.objects.create(
                                                ...
                                                target_user=invitation.creator,
                                                action="email_failed",
                                                description=f"Error enviando email a {docente_email}: {e}",
                                            )
                                        except Exception:
                                            pass
                            except Exception:
                ...
                try:
                    UserProfile.objects.create(
                        user=user, rol=UserProfile.Roles.DOCENTE, esta_activo=False
                    )
                except Exception:
    ...
    class Meta:
        ...
        widgets = {
            "nombre": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Ej: Matemáticas 2025-1"}
            ),
